...
use xcm_executor::{
	traits::{ConvertLocation as MoreConvert, MatchesFungible, TransactAsset},
	Assets,
};
...
impl<
		...
		AccountId: sp_std::fmt::Debug + Clone,
		AccountIdConvert: MoreConvert<AccountId>,
		CurrencyId: FullCodec + Eq + PartialEq + Copy + MaybeSerializeDeserialize + Debug,
	...
	>
{
	...
	fn transfer_asset(
		...
		let from_account =
			AccountIdConvert::convert_location(from).ok_or(XcmError::from(Error::AccountIdConversionFailed))?;
		let to_account =
			AccountIdConvert::convert_location(to).ok_or(XcmError::from(Error::AccountIdConversionFailed))?;
		let currency_id = CurrencyIdConvert::convert(asset.clone())
			.ok_or_else(|| XcmError::from(Error::CurrencyIdConversionFailed))?;
		let amount: MultiCurrency::Balance = Match::matches_fungible(asset)
			.ok_or_else(|| XcmError::from(Error::FailedToMatchFungible))?
